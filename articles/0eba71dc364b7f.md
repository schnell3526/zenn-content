...
HuggingFace TransformersのTrainerクラスを使って学習する場合、Trainerクラスの`deepspeed=`パラメタに以下の`ds_config.json`のパスを渡し、起動コマンドを`torchrun`や`python`から`deepspeed`に変更することで実現できます。DeepSpeedを利用することで本来メモリに乗り切らない巨大なモデルでも学習することができます。
```json
{
    "fp16": {
        "enabled": "auto",
...
学習時のパラメタに関しては大まかには以下のようになっています。詳しくは[こちら](https://huggingface.co/nlp-waseda/gpt2-xl-japanese/tensorboard)を参考にしてください。
```yaml
...
パイプラインを使って簡単に試してみます。入力するテキストは事前にjuman++によるトークナイズ処理が行われているものが望ましいですが、分かち書きなしでもある程度は動かせます。
